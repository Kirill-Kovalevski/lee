document.addEventListener("DOMContentLoaded", () => {
  const box  = document.querySelector(".flex-search-container"); // the container to move
  const slot = document.getElementById("search-slot");           // mobile target slot
  if (!box || !slot) return console.warn("Missing .flex-search-container or #search-slot");

  // Remember original position
  const placeholder = document.createComment("flexSearchPH");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const FLEX = document.querySelector('.flex-search-container');          // e.g. '.flex-container' or '#row'
  const ITEM = document.querySelector('.filter-text');          // e.g. '.box2' or '#right'

  const BREAKPOINT = 430; // px

  if (!FLEX || !ITEM) return; // nothing to do if selectors aren't found

  // Keep original position so we can restore in the same spot
  const placeholder = document.createComment('removedItemPH');
...
//Boxes shift 
document.addEventListener("DOMContentLoaded", () => {
  const box   = document.querySelector(".flex-search-container"); // source
  const slot1 = document.getElementById("block-top");            // top target
  const slot2 = document.getElementById("block-bottom");         // bottom target
  if (!box || !slot1 || !slot2) return;

  // First two children
  const el1 = box.children[0]; // originally first
  const el2 = box.children[1]; // originally second
  if (!el1 || !el2) return;

  // Placeholders to restore originals
  const ph1 = document.createComment("el1PH");
  const ph2 = document.createComment("el2PH");
